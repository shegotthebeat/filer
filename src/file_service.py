#!/usr/bin/env python3
"""
Personal File Upload Service
Minimalist file upload/download service for Raspberry Pi 5

Storage: /mnt/storage/uploads/
Port: 5001
"""
...
<body>
    <div class="container">
        <div class="sidebar">
            <h1>File Hub</h1>
                ...
                <div class="nav-item active">
                    <a href="https://salamander.blue">CSV Formatter</a>
                </div>
            ...
            <div class="storage-info">
                Storage<br>
                {{ storage_info.free_gb }}GB free<br>
                {{ storage_info.total_gb }}GB total
            </div>
        </div>
    ...
    <script>
        ...
        if (isMobile) {
            ...
            document.querySelector('#mobileForm input[type="file"]').addEventListener('change', function() {
                const files = this.files;
                if (files.length > 0) {
                    showStatus('mobileStatus', `Selected ${files.length} file(s)`, 'success');
                }
            });
        } else {
        ...
        function downloadFromUrl() {
            ...
            if (!url) {
                showStatus('urlStatus', 'Enter URL', 'error');
                return;
            }
            ...
            .catch(error => {
                showStatus('urlStatus', 'Download failed', 'error');
            });
        }
...
def index():
    ...
    try:
        ...
        storage_info = {'total_gb': 0, 'free_gb': 0}
    ...
    try:
        ...
        <body>
            <div class="container">
                ...
                <div class="file-list">
                    <ul>
                        {"".join(f"<li>{name}</li>" for name in uploaded_names)}
                    </ul>
                </div>
                <div class="actions">
...
def list_files():
    ...
    for file in files:
        try:
            ...
            file_list += f'''
                    <div class="file-entry">
                        <a href="/download/{file}" class="file-name">{display_name}</a>
                        <div class="file-meta">{size:.1f}MB • {date}</div>
                    </div>
            '''
        except:
            continue
    ...
    file_list += '''
        ...
        <script>
            ...
            .then(response => response.json())
            .then(data => {
                document.querySelector('.storage-info').innerHTML = `Storage<br>${data.free_gb}GB free<br>${data.total_gb}GB total`;
            })
            .catch(() => {
                document.querySelector('.storage-info').innerHTML = 'Storage<br>Info unavailable';
            });
        </script>
    </body>
    </html>
